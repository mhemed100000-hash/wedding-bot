...
iq_comments = {
    (90, 100): [
        "يا زلمة هاد عبقري مو طبيعي! واضح إنه بيقرأ وبيدرس وبيفكر، بس خلينا نشوف شو بيعمل بهالذكاء، لأنه لحد هلق ما شفنا شي 😂🧠",
        "هاد ذكاؤه فوق المعدل بكتير، بس المشكلة إنه بيستخدمه بأشياء ما بتنفع 😂 الله يعين اللي بيتعاملوا معه!",
        "عبقري بامتياز، بس خايف يعترف لأنه ما بده حدا يطلب منه مساعدة 😂🧠",
    ],
    (70, 89): [
        "مو بطال والله! ذكاؤه فوق المتوسط بس بيتظاهر إنه ما بيعرف شي لأنه كسلان يشرح 😂 لو اشتغل على حاله كان وصل لأماكن كتير 🧠",
        "شاطر بس مو بالدرجة اللي بيتخيلها عن نفسه 😏 يعني منيح بس ما تكبّر راسك كتير 😂",
        "ذكي بس بيستخدم ذكاؤه بالتفلسف والحكي الفاضي أكثر من الشغل الحقيقي 😂🧠",
    ],
    (50, 69): [
        "متوسط يا صديقي، لا ذكي ولا غبي، بالضبط متل الطالع والنازل 😅 بتفهم الأشياء بس بعد ما يشرحولك مرتين وتسأل مرة تالتة 😂",
        "نص نص والله، مرة بيفهم ومرة بيحتاج شرح إضافي 😅 بس ما عليك، المهم النية الحلوة 😂",
        "ذكاؤه موجود بس نايم، لو صحّى شوي كان أحسن بكتير 😂🧠",
    ],
    (30, 49): [
        "يا حبيبي والله ما بعرف كيف بتتصرف بحياتك اليومية 😂 بتحتاج شرح مطوّل لأبسط الأشياء وبعدين بتقول فهمت وأنت ما فهمت شي 😅",
        "أقل من المتوسط بشوي، بس ما تزعل، في ناس أقل منك وبعيشوا منيح 😂 المهم إنك طيب القلب حتى لو الدماغ مو معك كتير 😅",
        "هاد بيحتاج دورات تدريبية عاجلة 😂 الله يعين أصحابه اللي بيشرحولوه كل شي 🧠",
    ],
    (0, 29): [
        "يا زلمة والله ما توقعنا هالرقم 😂 بس لما فكرنا أكثر قلنا طبيعي منك، انت اللي بتحكي وبتضحك على نكتة بعد ساعة لما تفهمها 😂 الله يعين اللي بيحاولوا يفهموا عليك!",
        "هاد الرقم بيقول كل شي 😂 بس خلينا نكون إيجابيين، ممكن الذكاء عندك بمكان تاني مو بالدماغ 😅 متل الأكل مثلاً!",
        "صفر على الشمال تقريباً 😂 بس لا تحزن، انت مسلّي وهاد أهم من الذكاء أحياناً 😅🤡",
    ],
}

def get_iq_bar(percentage):
    smart = percentage // 20
    crazy = (percentage % 20) // 10
    rest = 10 - smart - crazy
    return "🧠" * smart + "🤪" * crazy + "🤡" * min(rest, 5) + "💀" * max(rest - 5, 0)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "زوجني":
        ...
        percentage = random.randint(0, 100)
        bar = get_iq_bar(percentage)
        for (low, high), comments in iq_comments.items():
            if low <= percentage <= high:
                comment = random.choice(comments)
                break
        await update.message.reply_text(
            f"🧠 تقرير الذكاء الرسمي والنهائي!\n\n"
            f"الاسم: {user_name}\n\n"
            f"{bar} {percentage}%\n\n"
            f"{comment}",
            parse_mode="Markdown"
