...
def attacker_display_name(country):
    name = country.get("اسم", "?")
    alliance = country.get("تحالف")
    if alliance:
        return f"{name} (تحالف: {alliance})"
    return name


async def notify_attack_to_user_or_alliance(bot, chat_id, target_name, attacker_label, weapon_label, damage=None, amount=None, prefer_alliance=False):
    """إرسال إشعار خاص عند الهجوم على دولة أو تحالف."""
    if not prefer_alliance:
        target_country = get_country_by_name(target_name)
        if target_country and target_country.get("user_id"):
            message = (
                f"🚨 تم الهجوم عليك!\n\n"
                f"👤 المهاجم: {attacker_label}\n"
                f"🎯 الهدف: {target_name}\n"
                f"🚀 النوع: {weapon_label}\n"
            )
            if amount is not None:
                message += f"📦 العدد: {amount}\n"
            if damage is not None:
                message += f"💥 الضرر: {damage}"
            try:
                await bot.send_message(chat_id=target_country["user_id"], text=message)
            except:
                pass
            return
    ...
    members = list(target_alliance.get("أعضاء", []))
    message = (
        f"🚨 تم الهجوم على تحالفك!\n\n"
        f"👤 المهاجم: {attacker_label}\n"
        f"🎯 الهدف: {target_alliance['اسم']}\n"
        f"🚀 النوع: {weapon_label}\n"
    )
    if amount is not None:
        message += f"📦 العدد: {amount}\n"
    ...
    for member_name in members:
        ...
        if member_country and member_country.get("user_id"):
            try:
                await bot.send_message(chat_id=member_country["user_id"], text=message)
            except:
                pass
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("هاجم "):
        ...
        if attack_power > defense_power:
            ...
            save_country(attacker)
            save_country(defender)
            await notify_attack_to_user_or_alliance(context.bot, chat_id, target_name, attacker_display_name(attacker), "هجوم بري", damage=None, amount=None)
            await reply(
                f"⚔️ *نتيجة المعركة* ⚔️\n━━━━━━━━━━━━━━━\n"
                f"🏴 المهاجم: *{attacker['اسم']}*\n🏳️ المدافع: *{target_name}*\n"
                f"{defense_note}\n\n"
    ...
    if text.startswith("نووي "):
        ...
        if intercepted:
            cap_gold(attacker)
            save_country(attacker)
            await notify_attack_to_user_or_alliance(context.bot, chat_id, target_name, attacker_display_name(attacker), "نووي", 0, amount=1)
            await reply(
        ...
        save_country(attacker)
        save_country(defender)
        await notify_attack_to_user_or_alliance(context.bot, chat_id, target_name, attacker_display_name(attacker), "نووي", int(nuke_power), amount=1)
    ...
    # === إطلاق الصواريخ المباشر ===
    if text.startswith("اطلق صاروخ "):
        parts = text.split()
        if len(parts) < 5:
            await reply("❌ الاستخدام: `اطلق صاروخ Scud على العراق`")
            return

        try:
            missile_idx = 2  # after "اطلق صاروخ"
            if "على" in parts:
                target_idx = parts.index("على")
            elif "علي" in parts:
                target_idx = parts.index("علي")
            else:
                await reply("❌ اكتب: `اطلق صاروخ [النوع] على [الدولة]`")
                return

            missile_type = " ".join(parts[missile_idx:target_idx]).strip()
            target_name = " ".join(parts[target_idx + 1:]).strip()
            if not missile_type or not target_name:
                await reply("❌ اكتب: `اطلق صاروخ [النوع] على [الدولة]`")
                return
        except ValueError:
            await reply("❌ اكتب: `اطلق صاروخ [النوع] على [الدولة]`")
        ...
        if get_country_by_name(target_name) and get_country_by_name(target_name)["user_id"] == uid:
            await reply("❌ لا يمكنك مهاجمة نفسك!")
            return

        if missile_type not in UNITS or UNITS[missile_type]["نوع"] != "صواريخ":
            await reply("❌ هذا النوع ليس صاروخاً أو غير موجود!")
            return

        if attacker.get("وحدات", {}).get(missile_type, 0) < 1:
            await reply(f"❌ لا تملك صاروخ *{missile_type}*!", main_menu_keyboard(uid))
            return

        attacker = update_resources(attacker)
        target_country = get_country_by_name(target_name)
        target_alliance = load_alliance(chat_id, target_name)
        if not target_country and not target_alliance:
            await reply(f"❌ لا توجد دولة أو تحالف باسم `{target_name}`!")
            return

        # cooldown مباشر للصواريخ
        now = time.time()
        last_missile = attacker.get("آخر_صاروخ", 0)
        missile_cd = 60
        if now - last_missile < missile_cd:
            remaining = int(missile_cd - (now - last_missile))
            await reply(f"⏳ انتظر {remaining} ثانية قبل الإطلاق مرة أخرى!")
            return

        # هجوم بسيط متوازن
        missile_power = UNITS[missile_type]["قوة"]
        success = random.randint(1, 100) <= 75
        damage = int(missile_power * (1.25 if success else 0.35))

        if target_country:
            if is_protected(target_country):
                remaining = max(1, int((target_country["حماية حتى"] - time.time()) / 60))
                await reply(f"🛡️ *{target_name}* محمية لمدة {remaining} دقيقة!")
                return
            defense = target_country.get("دفاع", 0)
            damage = max(0, damage - defense)
        else:
            # إذا كان الهدف تحالفاً، نخفض الضرر حسب متوسط دفاع أعضائه
            total_def = 0
            members = target_alliance.get("أعضاء", []) if target_alliance else []
            for m_name in members:
                m = get_country_by_name(m_name)
                if m:
                    total_def += calc_air_defense_power(m.get("وحدات", {}))
            if members:
                damage = max(0, damage - int(total_def / max(1, len(members)) * 0.2))

        attacker["وحدات"][missile_type] -= 1
        attacker["آخر_صاروخ"] = now
        save_country(attacker)

        attacker_label = attacker_display_name(attacker)

        await notify_attack_to_user_or_alliance(
            context.bot,
            chat_id,
            target_name,
            attacker_label,
        ...
        result_text = "✅ نجاح" if success else "❌ فشل"
        await reply(
            f"🚀 *إطلاق صاروخ مباشر*\n"
            f"━━━━━━━━━━━━━━━\n"
            f"🎯 الهدف: *{target_name}*\n"
    ...
    if att_roll > def_roll:
        ...
        for m_name in alliance.get("أعضاء", []):
            ...
            if m:
                ...
                cap_gold(m)
                save_country(m)
        await notify_attack_to_user_or_alliance(bot, chat_id, target_name, alliance_name, "هجوم تحالف", damage=None, amount=None, prefer_alliance=True)
