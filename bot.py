...
def war_keyboard(user_id=None):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔥 هجوم تحالف", callback_data=f"how_to_alliance_attack{suffix}")],
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === نظام أمان الأزرار ===
    # الأزرار المفتوحة للجميع (تصويت)
    if data.startswith("vote_yes_") or data.startswith("vote_no_"):
        pass  # يتم التحقق لاحقاً داخل منطق التصويت
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("اشتري "):
        ...
        if unit_name not in UNITS:
            await reply(f"❌ الوحدة `{unit_name}` غير موجودة!", reply_markup=main_menu_keyboard(uid))
            return
...
async def execute_alliance_attack(context):
    ...
    if def_alliance and collective_def > 0:
        ...
        def_note = f"🛡️ دفاع تحالف *{def_alliance['اسم']}* الجماعي (+20%)"
    elif defender:
