...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("alliance_attack_list_"):
        ...
        for a in page_alliances:
            ...
            buttons.append([InlineKeyboardButton(btn_text, callback_data=f"aa_target_{a['اسم']}|{uid}")])
        # أزرار التنقل
        nav_row = []
        if page > 0:
    ...
    # === بدء هجوم تحالف على تحالف مختار ===
    if data.startswith("aa_target_"):
        target_alliance_name = data.split("aa_target_")[1]
        country = load_country(uid)
        if not country:
            await query.edit_message_text("❌ ليس لديك دولة!", reply_markup=back_war_kb)
            return
    ...
    if data.startswith("kick_member_list_"):
        ...
        for m_name in page_members:
            ...
            buttons.append([InlineKeyboardButton(btn_text, callback_data=f"kick_confirm_{m_name}|{uid}")])
        nav_row = []
        if page > 0:
    ...
    if data.startswith("kick_confirm_"):
        target_name = data.split("kick_confirm_")[1]
        country = load_country(uid)
        if not country:
            await query.edit_message_text("❌ ليس لديك دولة!", reply_markup=back_alliance_kb)
            return
        ...
        confirm_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ تأكيد الطرد", callback_data=f"kick_do_{target_name}|{uid}"),
    ...
    # === تنفيذ الطرد ===
    if data.startswith("kick_do_"):
        target_name = data.split("kick_do_")[1]
        country = load_country(uid)
        if not country:
            await query.edit_message_text("❌ ليس لديك دولة!", reply_markup=back_alliance_kb)
            return
    ...
    if data.startswith("vote_yes_") or data.startswith("vote_no_"):
        ...
        is_leader_voted_yes = alliance.get("قائد") in vote.get("أصوات_نعم", [])
