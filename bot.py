...
def run_phased_battle(attacker, defender, chat_id=None):
    ...
    if att_ground_roll > 0 or def_ground_roll > 0:
        # حساب فارق القوة — الطرف الأقوى بكثير يخسر أقل
        if att_ground > 0 and def_ground > 0:
            ratio = max(att_ground, def_ground) / max(min(att_ground, def_ground), 1)
        else:
            ratio = 1
        
        if ground_winner == "المهاجم":
            # الفائز يخسر أقل كل ما كان أقوى
            if ratio >= 10:
        ...
        if "مستشفى عسكري" in attacker.get("مباني", []):
            att_g_loss_pct = max(5, att_g_loss_pct - 8)
        if "مستشفى عسكري" in defender.get("مباني", []):
            def_g_loss_pct = max(5, def_g_loss_pct - 8)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("atk_"):
        ...
        # التحقق أن المهاجم عنده قوات برية أو جوية
        att_ground = calc_type_count(attacker.get("وحدات", {}), "مشاة") + calc_type_count(attacker.get("وحدات", {}), "مدرعات")
        att_air = calc_type_count(attacker.get("وحدات", {}), "جوي")
        if att_ground == 0 and att_air == 0:
            await query.edit_message_text("❌ لا تملك قوات برية أو جوية للهجوم!\n\n💡 الصواريخ والنووي لا تُستخدم في الهجوم البري.", parse_mode="Markdown", reply_markup=back_war_kb)
            return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("هاجم "):
        ...
        # التحقق أن المهاجم عنده قوات برية أو جوية للهجوم
        att_ground = calc_type_count(attacker.get("وحدات", {}), "مشاة") + calc_type_count(attacker.get("وحدات", {}), "مدرعات")
        att_air = calc_type_count(attacker.get("وحدات", {}), "جوي")
        if att_ground == 0 and att_air == 0:
            await reply("❌ لا تملك قوات برية أو جوية للهجوم!\n\n💡 الصواريخ والنووي لا تُستخدم في الهجوم البري.\nاستخدم: `اطلق صاروخ` أو `نووي`")
            return
