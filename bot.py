...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("هاجم "):
        ...
        def_alliance, collective_def = calc_collective_defense(chat_id, target_name)
        if def_alliance and collective_def > 0:
            defense_power = collective_def
            defense_note = f"🛡️ دفاع تحالف *{def_alliance['اسم']}* الجماعي (+20%)"
        else:
            defense_power = calc_power(defender.get("وحدات", {}))
            defense_note = ""

        attack_power = calc_power(attacker.get("وحدات", {}))

        attacker_air = sum(
            UNITS[u]["قوة"] * c for u, c in attacker.get("وحدات", {}).items()
            if u in UNITS and UNITS[u]["نوع"] == "جوي"
        )
        defender_air_def = calc_air_defense_power(defender.get("وحدات", {}))
        attack_power += attacker_air * 0.10
        attack_power -= min(defender_air_def * 0.30, attacker_air * 0.45)
        ...
        success = random.random() < win_prob

        if success:
            stolen_gold = int(defender.get("ذهب", 0) * min(0.35, 0.18 + min(ratio, 3) * 0.05))
            attacker["ذهب"] = attacker.get("ذهب", 0) + stolen_gold
            defender["ذهب"] = max(0, defender.get("ذهب", 0) - stolen_gold)
            attacker["انتصارات"] = attacker.get("انتصارات", 0) + 1
            defender["خسائر"] = defender.get("خسائر", 0) + 1
            if "مستشفى عسكري" in attacker.get("مباني", []):
                att_loss = max(5, att_loss - 8)
            if "مستشفى عسكري" in defender.get("مباني", []):
                def_loss = max(8, def_loss - 8)
            for unit in attacker.get("وحدات", {}):
                attacker["وحدات"][unit] = int(attacker["وحدات"][unit] * (1 - att_loss / 100))
            for unit in defender.get("وحدات", {}):
                defender["وحدات"][unit] = int(defender["وحدات"][unit] * (1 - def_loss / 100))
            grant_shield(defender, SHIELD_DURATION)
            cap_gold(attacker)
            save_country(attacker)
            save_country(defender)
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=notify_attack_text(build_attacker_label(attacker), "هجوم بري")
                )
            except:
                pass
            ...
            defender["انتصارات"] = defender.get("انتصارات", 0) + 1
            if "مستشفى عسكري" in attacker.get("مباني", []):
                att_loss = max(20, att_loss - 8)
            if "مستشفى عسكري" in defender.get("مباني", []):
                def_loss = max(5, def_loss - 6)
            for unit in attacker.get("وحدات", {}):
                attacker["وحدات"][unit] = int(attacker["وحدات"][unit] * (1 - att_loss / 100))
            for unit in defender.get("وحدات", {}):
                defender["وحدات"][unit] = int(defender["وحدات"][unit] * (1 - def_loss / 100))
