...
async def notify_defender_about_attack(bot, defender_country, attacker_country, weapon_label, amount=None, damage=None):
    """إشعار موحد لأي هجوم عادي."""
    if not defender_country or not defender_country.get("user_id"):
        return
    lines = [
        "🚨 تم الهجوم عليك!",
        "",
        f"👤 المهاجم: {build_attacker_label(attacker_country)}",
    ...
    try:
        await bot.send_message(chat_id=defender_country["user_id"], text="\n".join(lines))
    except:
        pass

async def notify_alliance_about_attack(bot, target_alliance, attacker_country, weapon_label, amount=None, damage=None):
    """إشعار لجميع أعضاء التحالف عند الهجوم عليه."""
    if not target_alliance:
        return
    lines = [
        "🚨 تم الهجوم على تحالفك!",
        "",
        f"👤 المهاجم: {build_attacker_label(attacker_country)}",
        f"🎯 النوع: {weapon_label}",
    ]
    ...
    for member_name in target_alliance.get("أعضاء", []):
        member_country = get_country_by_name(member_name)
        if member_country and member_country.get("user_id"):
            try:
                await bot.send_message(chat_id=member_country["user_id"], text=text)
            except:
                pass
...
UNITS = {
    ...
    # === صواريخ ===
    "Grad": {"قوة": 200, "سعر": 3000, "نوع": "صواريخ"},
    "كروز": {"قوة": 400, "سعر": 7500, "نوع": "صواريخ"},
    "Scud": {"قوة": 550, "سعر": 12500, "نوع": "صواريخ"},
    "باتريوت": {"قوة": 60, "سعر": 15000, "نوع": "دفاع"},
    "Iskander": {"قوة": 750, "سعر": 20000, "نوع": "صواريخ"},
    "Tomahawk": {"قوة": 850, "سعر": 30000, "نوع": "صواريخ"},
    "ICBM": {"قوة": 1000, "سعر": 50000, "نوع": "صواريخ"},
    # === أسلحة جديدة: صواريخ ===
    "MLRS": {"قوة": 450, "سعر": 10000, "نوع": "صواريخ"},
    "Iskander-M": {"قوة": 850, "سعر": 25000, "نوع": "صواريخ"},
    "Kinzhal": {"قوة": 900, "سعر": 60000, "نوع": "صواريخ"},
    "Tupolev": {"قوة": 1000, "سعر": 80000, "نوع": "صواريخ"},
    # === نووي ===
...
UNIT_ALIASES = {
    ...
    "tupolev": "Tupolev", "توبوليف": "Tupolev", "tu": "Tupolev",
...
def calc_missile_intercept_chance(defender, missile_type):
    """احتمال اعتراض الصاروخ حسب قوة الدفاع ونوع الصاروخ"""
    ad_power = calc_air_defense_power(defender.get("وحدات", {}))
    base = min(0.88, 1 - math.exp(-ad_power / 700))
    missile_factor = {
        "Grad": 1.00,
        "كروز": 0.92,
        "Scud": 0.88,
...
def main_menu_keyboard(user_id=None):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🪖 جيشي", callback_data=f"my_army{suffix}"),
         InlineKeyboardButton("📋 مهماتي", callback_data=f"missions{suffix}")],
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "buy_protection":
        ...
        country = update_resources(country)
        today = datetime.now().date().isoformat()
        if country.get("تاريخ_الحماية") != today:
            country["تاريخ_الحماية"] = today
            country["استخدام_الحماية_اليومي"] = 0
        if country.get("استخدام_الحماية_اليومي", 0) >= DAILY_PROTECTION_LIMIT:
            await query.edit_message_text(
                f"❌ لقد وصلت للحد اليومي للحماية ({DAILY_PROTECTION_LIMIT}/7)\n"
                f"عد غداً لتفعيل الحماية مجدداً.",
                reply_markup=back_markets_kb
            )
            return
        ...
        country["استخدام_الحماية_اليومي"] = country.get("استخدام_الحماية_اليومي", 0) + 1
        cap_gold(country)
        save_country(country)
        await query.edit_message_text(
            f"🛡️ *تم شراء الحماية!*\n\n"
            f"أنت محمي لمدة ساعة واحدة! 💪\n"
            f"المحاولات المتبقية اليوم: {max(0, DAILY_PROTECTION_LIMIT - country['استخدام_الحماية_اليومي'])}",
            parse_mode="Markdown", reply_markup=back_main_kb
        )
        return
    ...
    if data == "how_to_nuke":
        await query.edit_message_text(
            "☢️ *الضربة النووية*\n━━━━━━━━━━━━━━━\n\n"
            "اكتب: `نووي [اسم الدولة]`\nأو `نووي تكتيكي [اسم الدولة]`\nأو `كيميائي [اسم الدولة]`\n\n"
            "📌 *الشروط:*\n"
            "• تحتاج منشأة نووية\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("انشاء دولة "):
        ...
        new_country = {
            ...
            "استخدام_الحماية_اليومي": 0,
            "تاريخ_الحماية": datetime.now().date().isoformat(),
    ...
    if text.startswith("هاجم "):
        ...
        if attack_power > defense_power:
            ...
            save_country(attacker)
            save_country(defender)
            await notify_defender_about_attack(context.bot, defender, attacker, "هجوم بري", damage=def_loss)
            await reply(
                f"⚔️ *نتيجة المعركة* ⚔️\n━━━━━━━━━━━━━━━\n"
                f"🏴 المهاجم: *{attacker['اسم']}*\n🏳️ المدافع: *{target_name}*\n"
                f"{defense_note}\n\n"
            ...
            save_country(attacker)
            save_country(defender)
            await notify_defender_about_attack(context.bot, defender, attacker, "هجوم بري", damage=att_loss)
            await reply(
                f"⚔️ *نتيجة المعركة* ⚔️\n━━━━━━━━━━━━━━━\n"
                f"🏴 المهاجم: *{attacker['اسم']}*\n🏳️ المدافع: *{target_name}*\n"
                f"{defense_note}\n\n"
    ...
    # === نظام النووي ===
    if text.startswith("نووي تكتيكي ") or text.startswith("كيميائي ") or text.startswith("نووي "):
        if text.startswith("نووي تكتيكي "):
            weapon_mode = "tactical"
            target_name = text[len("نووي تكتيكي "):].strip()
        elif text.startswith("كيميائي "):
            weapon_mode = "chemical"
            target_name = text[len("كيميائي "):].strip()
        else:
            weapon_mode = "strongest"
            target_name = text[len("نووي "):].strip()
        ...
        if weapon_mode == "chemical":
            best_nuke = "كيميائي"
            if attacker.get("وحدات", {}).get(best_nuke, 0) <= 0:
                await reply("❌ لا تملك أي سلاح كيميائي!")
                return
        elif weapon_mode == "tactical":
            best_nuke = "نووي تكتيكي"
            if attacker.get("وحدات", {}).get(best_nuke, 0) <= 0:
                await reply("❌ لا تملك أي نووي تكتيكي!")
                return
        else:
        ...
        if intercepted:
            cap_gold(attacker)
            save_country(attacker)
            await notify_defender_about_attack(context.bot, defender, attacker, "نووي", amount=1, damage=0)
            await reply(
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    f"🚨 تم الهجوم عليك!\n\n"
                    f"👤 المهاجم: {build_attacker_label(attacker)}\n"
                    f"🎯 النوع: {best_nuke}\n"
                    f"💥 الضرر: 80%\n"
                    f"🏗️ المبنى المدمر: {destroyed_building if destroyed_building else 'لا يوجد'}"
                )
            )
        except:
            pass
        ...
        await reply(
            ...
            f"🏴 المهاجم: *{attacker['اسم']}*\n🎯 الهدف: *{target_name}*\n"
            f"🚀 السلاح: *{best_nuke}* (قوة ×10 تقريباً)\n\n"
            f"📊 *تقرير الضربة:*\n"
    ...
    if text in ["اغادر التحالف", "غادر التحالف", "خروج من التحالف", "اخرج من التحالف", "مغادرة التحالف", "ترك التحالف"]:
        country = load_country(uid)
        if not country:
            await reply("❌ ليس لديك دولة!")
            return
        ...
        if country["اسم"] == alliance["قائد"] and len(alliance["أعضاء"]) > 1:
            await reply("❌ لا يمكنك الخروج وأنت القائد الأعلى!\nاستخدم `حذف التحالف` أو انقل القيادة أولاً.")
            return
        if country["اسم"] in alliance.get("أعضاء", []):
            alliance["أعضاء"].remove(country["اسم"])
        if country["اسم"] in alliance.get("جنرالات", []):
            alliance["جنرالات"].remove(country["اسم"])
        if country["اسم"] in alliance.get("عمداء", []):
    ...
    if text in ["حذف التحالف", "احذف التحالف", "حل التحالف"]:
        country = load_country(uid)
        if not country:
            await reply("❌ ليس لديك دولة!")
            return
        ...
        if not alliance:
            ...
            return
        if country["اسم"] != alliance.get("قائد"):
            await reply("🚫 *فقط القائد الأعلى* يمكنه حذف التحالف!")
            return
        db["join_requests"].delete_many({"chat_id": chat_id, "تحالف": alliance["اسم"]})
        db["votes"].delete_many({"chat_id": chat_id, "alliance_name": alliance["اسم"]})
        alliances_col.delete_one({"chat_id": chat_id, "اسم": alliance["اسم"]})
        await reply(f"🗑️ *تم حذف تحالف {alliance['اسم']} بالكامل!*", main_menu_keyboard(uid))
        return
