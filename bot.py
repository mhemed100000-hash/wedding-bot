...
def calc_nuke_intercept_chance(defender):
    ...
    chance = 1 - math.exp(-nuclear_def / 900)
    if has_air_def:
        chance += 0.08
    if has_research:
        chance += 0.04
    if is_secret_owner(defender):
        chance += 0.06  # أفضلية خفية ومحدودة لحسابك فقط
...
def run_phased_battle(attacker, defender, chat_id=None):
    ...
    if remaining_air_power > 0:
        ...
        report["مراحل"].append({
            ...
            "نتيجة": f"💥 دمّر {ground_damage_pct}% من قوات المدافع البرية",
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("نووي "):
        ...
        if is_secret_owner(defender):
            damage_ratio = max(0.12, damage_ratio * 0.90)

        nuke_power = UNITS[best_nuke]["قوة"] * 1.5

        if damage_ratio >= 0.80:
            destroyed_percent = 80
        elif damage_ratio >= 0.45:
            destroyed_percent = 45
        else:
            destroyed_percent = 20
        ...
        if defender.get("مباني"):
            if destroyed_percent >= 45:
                destroyed_building = random.choice(defender["مباني"])
                defender["مباني"].remove(destroyed_building)

        stolen = int(defender.get("ذهب", 0) * (0.40 if destroyed_percent == 80 else 0.20 if destroyed_percent >= 45 else 0.05))
        attacker["ذهب"] += stolen
